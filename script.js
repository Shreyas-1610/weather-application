...
function getWeatherDetails(name, lat, lon, country, state){
    ...
    AIR_POPULATION_API_URL = `https://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=${api_key}`,
    days = [
        'Sunday',
        'Monday',
        'Tuesday',
    ...
    fetch(AIR_POPULATION_API_URL).then(res => res.json()).then(data =>{
        let {co, no, o3, so2, pm2_5, pm10, nh3, no2} = data.list[0].components;
        aqiCard.innerHTML = `
        <div class="card-head">
